...
\hypertarget{query-1}{%
\subparagraph{Query:}\label{query-1}}

(ALL=(biomedical) OR ALL=(health)) AND (ALL=(repeated measures) OR
ALL=(longitudinal study) OR ALL=(longitudinal data)) AND
(ALL=(statistical analyses) OR ALL=(statistical analysis)) AND
(DT=(Article)) NOT (WC=(Statistics Probability) OR WC=(Mathematics)) NOT
(SU=(Agriculture) AND SU=(Business Economics) AND SU=(Veterinary
Sciences) AND SU=(Education Educational Research) AND SU=(Business
Economics) AND SU=(Social Sciences Other Topics) AND SU=(Food Science
Technology) AND SU=(Anthropology) AND SU=(Linguistics) AND
SU=(Sociology) AND SU=(Criminology Penology) AND SU=(Zoology) AND
SU=(Meteorology Atmospheric Sciences) AND SU=(Mathematical Methods in
Social Sciences) AND SU=(Geology) AND SU=(Construction Building
Technology) AND SU=(Geology) AND SU=(Religion) AND SU=(Marine Freshwater
Biology) AND SU=(Operations Research Management Science) AND
SU=(Fisheries) AND SU=(Metallurgy Metallurgical Engineering))
...
(ALL=(longitudinal studies) OR ALL=(repeated measures)) NOT
(TI=(survival)) AND (WC=(Statistics Probability) OR (WC=Mathematics) OR
